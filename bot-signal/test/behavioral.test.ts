...
describe("behavioral analysis", () => {
  ...
  it("detects a short programmed typing-delay template", () => {
    const repeated: KeySample[] = [{ t: 0, isTrusted: true }];
    const template = [73, 118, 91, 147, 84];
    let timestamp = 0;
    for (let index = 0; index < 20; index += 1) {
      timestamp += template[index % template.length];
      repeated.push({ t: timestamp, isTrusted: true });
    }

    const organicIntervals = [81, 139, 67, 204, 96, 154, 112, 73, 188, 125, 91, 231, 104, 146];
    const organic: KeySample[] = [{ t: 0, isTrusted: true }];
    timestamp = 0;
    for (const interval of organicIntervals) {
      timestamp += interval;
      organic.push({ t: timestamp, isTrusted: true });
    }

    expect(hasRepeatedTypingCadence(repeated)).toBe(true);
    expect(hasRepeatedTypingCadence(organic)).toBe(false);
  });
