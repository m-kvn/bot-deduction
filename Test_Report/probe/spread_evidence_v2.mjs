...
const STEALTH_INIT = () => {
  if (globalThis.__SKIP_WEBGL_PATCH) return;
...
async function clickField(page, state, selector) {
  ...
  await page.mouse.move(x + rand(-1.6, 1.6), y + rand(-1.6, 1.6));
...
await context.addInitScript(() => { globalThis.__SKIP_WEBGL_PATCH = true; });
await context.addInitScript(STEALTH_INIT);
// (worker realm left unpatched on purpose in run A; run B drops the patch entirely)
...
await page.mouse.down();
await sleep(rand(25, 60));
await page.mouse.move(bx + rand(-1.6, 1.6), by + rand(-1.6, 1.6));
await sleep(rand(25, 70));
await page.mouse.up();
